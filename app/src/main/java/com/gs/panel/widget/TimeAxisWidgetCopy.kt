...
//                            .border(1.dp, Color.Blue)
                ...
                //绘制已经过去的时间段
                val curProgress = (TimeUtil.getHour() * 60 + TimeUtil.getMinute()).toFloat()
                val remindProgress = (1440 - curProgress)
...
//                    .background(CustomColor.green)
                    ...
                    for (i in 0..24) {
                        Spacer(modifier = Modifier.width(2.dp).fillMaxHeight().background(borderColor))
                    }
                }
            }
        }
